...
describe("POST - /article", () => {
    ...
    it("Ajouter article avec champs manquant - E", (done) => {
        chai.request(server).post('/article').auth(users[0].token,{type: 'bearer'}).send({
            name:"Aventador",
    ...
    it("Ajouter un article sans authentification - E", (done) => {
...
describe("POST - /articles", () => {
    it("Ajouter des articles - S", (done) => {
        ...
        chai.request(server).post('/articles').auth(users[0].token,{type: 'bearer'}).send([{
            name : "gazon premium",
            user_id : user_id,
    ...
    it("Ajouter des articles sans authentification - E", (done) => {
        const user_id = rdmUser(tab_id_users)
...
describe("GET - /article", () => {
    it("trouver un article via name correct - S", (done) => {
        chai.request(server).get('/article').query({fields : ["name"], value:article[0].name}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(200)
            done()
        })
    })
    it("trouver un article via name correct sans authentification- E", (done) => {
    ...
    it("trouver un article via name correct fields incorrect - E", (done) => {
        chai.request(server).get('/article').query({fields : "description", value:article[0].name}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(405)
            done()
        })
    })
    it("trouver un article via name correct fields correct mais article inexistant - E", (done) => {
        chai.request(server).get('/article').query({fields : ["name"], value:"turturututdfshiqergherqi"}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(404)
            done()
        })
    })
    it("trouver un article sans aucune query - E", (done) => {
        chai.request(server).get('/article').auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(405)
            done()
        })
    })
...
describe("GET - /article/:id", () => {
    it("trouver un article avec ID incorrect - E", (done) => {
        chai.request(server).get(`/article/66756baf835213e8d650c3ac`).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(404)
            done()
        })
    })
    it("trouver un article avec ID incorrect - E", (done) => {
        chai.request(server).get(`/article/66756680`).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(405)
            done()
        })
    })
    it("trouver un article correct - S", (done) => {
        chai.request(server).get(`/article/${article[0]._id}`).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(200)
            done()
        })
    })
    it("trouver un article correct sans authentification- E", (done) => {
...
describe("GET - /articles_by_filter", () => {
    it("trouver les articles avec query correct - S", (done) => {
        chai.request(server).get('/articles_by_filter').query({page:1,limit:3}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(200)
    ...
    it("trouver les articles avec query correct sans authentification - E", (done) => {
    ...
    it("trouver les articles de la page 3 -S ", (done) => {
        chai.request(server).get('/articles_by_filter').query({page:3,limit:3}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(200)
            expect(res.body.results).to.be.an('array')
            expect(res.body.count).to.be.equal(3)
            done()
    ...
    it("trouver les articles avec page manquant -S", (done) => {
        chai.request(server).get('/articles_by_filter').query({limit:3}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(200)
            expect(res.body.results).to.be.an('array')
            expect(res.body.count).to.be.equal(3)
            done()
    ...
    it("trouver les articles avec limit chaine de caractère -E", (done) => {
        chai.request(server).get('/articles_by_filter').query({page:"coucou", limit:3}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(405)
            done()
        })
    })
...
describe("GET - /articles", () => {
    it("trouver des articles avec id correct non présent - E", (done) => {
        chai.request(server).get(`/articles`).query({id:["66756baf835213e8d650c3ac"]}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(404)
            done()
        })
    })
    it("trouver des articles avec id inccorect- E", (done) => {
        chai.request(server).get(`/articles`).query({id:["fdrtdrt"]}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(405)
            done()
        })
    })
    it("trouver des articles corrects - S", (done) => {
        chai.request(server).get(`/articles`).query({id:[articles[0][0]._id,articles[0][1]._id]}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(200)
            done()
        })
    })
    it("trouver des articles corrects sans authentification- E", (done) => {
...
describe("UPDATE - /article", () => {
    it("modifier un article avec ID correct - S", (done) => {
        chai.request(server).put(`/article/${article[0]._id}`).auth(users[0].token,{type: 'bearer'}).send({
            name : "kiwi"
        }).end((err, res) => {
    ...
    it("modifier un article avec ID correct non présent - E", (done) => {
        chai.request(server).put('/article/667576cd3faa36454139a6a7').auth(users[0].token,{type: 'bearer'}).send({
            name : "kiwi"
        }).end((err, res) => {
            res.should.has.status(404)
            done()
    ...
    it("modifier un article avec ID incorrect - E", (done) => {
        chai.request(server).put('/article/667576c').auth(users[0].token,{type: 'bearer'}).send({
            name : "kiwi"
        }).end((err, res) => {
            res.should.has.status(405)
            done()
...
describe("UPDATE - /articles", () => {
    it("modifier des articles avec ID correct - S", (done) => {
        chai.request(server).put(`/articles`).query({id:[articles[0][0]._id,articles[0][1]._id]}).auth(users[0].token,{type: 'bearer'}).send({
    ...
    it("modifier des articles avec ID correct sans authentification - E", (done) => {
    ...
    it("modifier des articles avec ID correct mais non présent - E", (done) => {
        chai.request(server).put(`/article$s`).query({id:['667576cd3faa36454139a6a7',articles[0][1]._id]}).auth(users[0].token,{type: 'bearer'}).send({
            name : "princessePeatch"
        }).end((err, res) => {
            res.should.has.status(404)
            done()
    ...
    it("modifier des articles avec ID incorrect - E", (done) => {
        chai.request(server).put(`/articles`).query({id:['667576cd3',articles[0][1]._id]}).auth(users[0].token,{type: 'bearer'}).send({
            name : "princessePeatch"
        }).end((err, res) => {
            res.should.has.status(405)
            done()
...
describe("DELETE - /article", () => {
    it("supprimer un article avec ID incorrect - E", (done) => {
        chai.request(server).delete(`/article/66756680e15ea579048f90ef`).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(404)
            done()
        })
    })
    it("supprimer un article correct sans authentification- E", (done) => {
    ...
    it("supprimer un article correct - S", (done) => {
        chai.request(server).delete(`/article/${article[0]._id}`).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(200)
            done()
        })
    })
...
describe("DELETE - /articles", () => {
    it("supprimer des articles avec id correct non présent - E", (done) => {
        chai.request(server).delete(`/articles`).query({id:["667576cd3faa36454139a6a7"]}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(404)
            done()
        })
    })
    it("supprimer des articles avec id incorrect- E", (done) => {
        chai.request(server).delete(`/articles`).query({id:[true]}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(405)
            done()
        })
    })
    it("supprimer des articles corrects sans authentification - S", (done) => {
    ...
    it("supprimer des articles corrects - S", (done) => {
        chai.request(server).delete(`/articles`).query({id:[articles[0][0]._id,articles[0][1]._id]}).auth(users[0].token,{type: 'bearer'}).end((err, res) => {
            res.should.has.status(200)
            done()
        })
    })
